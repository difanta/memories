...
class TimelineQuery(private val mCtx: MainActivity) {
    ...
    fun getBackedUpSystemImages(): List<SystemImage> {
        ...
        val ids = mPhotoDao.getBackedUpPhotoIds(mConfigService.enabledBucketIds)
    ...
    fun getPendingRemoteCheck(): JSONArray {
        val r = mPhotoDao.getPendingPhotos(mConfigService.enabledBucketIds).map {
            JSONObject()
                .put(Fields.Photo.AUID, it.auid)
                .put(Fields.Photo.BUID, it.buid)
                .put(Fields.Photo.DAYID, it.dayId)
        }.let { JSONArray(it) }
        Log.v(TAG, "Pending remote check: ${r.length()} ${mConfigService.enabledBucketIds}")
        return r
    }
}