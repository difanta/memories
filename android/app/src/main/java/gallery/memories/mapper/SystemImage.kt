...
class SystemImage {
    ...
    companion object {
        ...
        fun cursor(
            ...
            ctx.contentResolver.query(
                ...
                while (cursor.moveToNext()) {
                    ...
                    // Attempt to parse EXIF date using ExifInterface created from content Uri when possible
                    //var dateCache = if (!cursor.isNull(dateTakenColumn)) cursor.getLong(dateTakenColumn) / 1000 else 0L
                    var instantZone: InstantZone? = null
                    try {
                        instantZone = parseExifDate(image.exifInterface, image.mimeType, dateTaken)
                    } catch (e: Exception) {
                        Log.w(TAG, "parseExifDate failed for id=${image.fileId}: ${e.message}")
                    }

                    // fill date taken (in s) with the exif date or fallback to mtime
                    var dateTakenInstant = instantZone?.instant ?: Instant.ofEpochSecond(image.mtime)

                    image.dateTaken = dateTakenInstant.getEpochSecond()

                    val dateTakenZdt = (instantZone?.zoneId ?: ZoneOffset.UTC).let { dateTakenInstant.atZone(it)  }
        ...
        fun parseExifDate(exif: ExifInterface?, mimeType: String?, dateTaken: Long?): InstantZone {
            ...
            if (bestInstant == null) {
                ...
                } else throw Exception("No parsable date found in EXIF or MediaStore")
            }
