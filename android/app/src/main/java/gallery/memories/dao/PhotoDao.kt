...
interface PhotoDao {
    ...
    @Query("SELECT local_id FROM photos WHERE has_remote = 1 AND bucket_id IN (:bucketIds)")
    fun getBackedUpPhotoIds(bucketIds: List<String>): List<Long>

    @Query("SELECT * FROM photos WHERE has_remote = 0 AND bucket_id IN (:bucketIds)")
    fun getPendingPhotos(bucketIds: List<String>): List<Photo>
}